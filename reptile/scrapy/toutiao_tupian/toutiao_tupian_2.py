...
def get_article_urls(req, timeout=10):
	...
	if d isNone:
		print("数据全部请求完毕...")
	return urls = [article.get('article_url') for article in d if article.get('article_url')]
return urls
def get_photo_urls(req, timeout=10):
	...
	# 所以我们使用 ignore 忽略这部分内容
		...
		ifnot article_main:
		print("无法定位到文章主体...")
		return heading = article_main.h1.string
	if'街拍'notin heading:
		print("这不是街拍的文章！！！")
	return img_list = [img.get('src') for img in article_main.find_all('img') if img.get('src')]
...
# 则 save_path = 'E：\jiepai\11125841455748.jpg'
	...
	with request.urlopen(photo_url, timeout=timeout) as res, save_path.open('wb') as f:
		f.write(res.read())
		print('已下载图片：{dir_name}/{photo_name}，请求的 URL 为：{url}'.format(dir_name=dir_name, photo_name=photo_name, url=a_url))
